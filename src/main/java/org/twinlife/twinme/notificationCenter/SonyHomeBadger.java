 ...
 * @author Leo Lin
 */
class SonyHomeBadger extends Badger {
    ...
    static class AsyncHandler extends AsyncQueryHandler {
        AsyncHandler(ContentResolver cr) {
            super(cr);
        }
    }

    SonyHomeBadger(Context context, ComponentName componentName) {
    ...
    protected void updateBadgeNumber(int badgeNumber) {
        if (sonyBadgeContentProviderExists(mContext)) {
            executeBadgeByContentProvider(mContext, mComponentName, badgeNumber);
        } else {
            executeBadgeByBroadcast(mContext, mComponentName, badgeNumber);
        }
    }
    ...
    private void executeBadgeByContentProvider(Context context, ComponentName componentName,
        ...
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            if (mQueryHandler == null) {
                mQueryHandler = new AsyncHandler(context.getApplicationContext().getContentResolver());
            }
     ...
     * @param contentValues Content values containing the badge count, package and activity names
     */
    private void insertBadgeSync(final ContentValues contentValues) {
